...
3. แสดงตัวเลขให้ชัดเจน - ใช้รูปแบบ XX,XXX.XX บาท สำหรับเงิน
...
- กรองเฉพาะ status ที่ไม่ใช่ 'UNPAID': `WHERE status::text != 'UNPAID'`
- วันที่: ใช้ `issue_at::date` เช่น `issue_at::date = CURRENT_DATE` สำหรับวันนี้
- ตัวอย่าง: `SELECT SUM(final_amount) FROM sell_invoice WHERE issue_at::date = CURRENT_DATE AND status::text != 'UNPAID'`
...
ตัวอย่างการตอบ:
    คำถาม: ยอดขายวันนี้เท่าไหร่
    คำตอบ: ยอดขายวันนี้ (15 ม.ค. 2568) อยู่ที่ 45,800.00 บาท จากทั้งหมด 12 รายการ
    
            สรุป:
                - รายการสูงสุด: 8,500 บาท
                - รายการต่ำสุด: 1,200 บาท
                - เฉลี่ยต่อรายการ: 3,816.67 บาท
"""